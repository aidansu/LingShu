...
def _convert_delta_to_message_chunk(
    ...
    if raw_tool_calls := _dict.get("tool_calls"):
        ...
        try:
            ...
            pass
    elif reasoning_content := _dict.get("reasoning_content"):
...
class LocalChatOpenAI(BaseChatOpenAI):  # type: ignore[override]
    """OpenAI chat model integration.
    
    新增参数:
        request_timeout: float, 请求超时时间（秒）
        max_retries: int, 最大重试次数
    """
    def __init__(self, *args, request_timeout: float = 60.0, max_retries: int = 3, **kwargs):
        # 兼容父类参数
